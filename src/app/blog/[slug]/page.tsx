...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${post.title} | Joplin Tree Service Blog`,
...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    headline: post.title,
    description: post.description,
    image: post.image ? [`https://joplinmotreeservice.com${post.image}`] : [],
    datePublished: post.date,
    author: {
      '@type': 'Person',
      name: post.author || 'Joplin Tree Service',
    },
  };
      ...
      <article className="flex-1 pt-32 pb-24">
        ...
        <header className="container mx-auto px-6 max-w-4xl text-center mb-16">
          <div className="inline-flex items-center gap-4 text-sm font-bold uppercase tracking-widest text-green-700 mb-6 border-b border-green-200 pb-2">
             ...
             <span className="flex items-center gap-2"><User className="w-4 h-4" /> {post.author || 'Joplin Tree Service'}</span>
          </div>
